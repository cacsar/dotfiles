...
class RubyFocusedUnitTest
  ...
  def write_output_to_buffer(test_command)
    ...
    buffer = VIM::Buffer.create DEFAULT_OUTPUT_BUFFER, :location => :below, :text => "--- #{test_command} ---\n\n"
  ...
  def run_unit_test
    ...
    write_output_to_buffer("ruby -I\"lib:test\" #{current_file} -n #{method_name}") if method_name
  end
  ...
  def run_context
    ...
    if method_name
      if spec_file?
        ...
        write_output_to_buffer("ruby -I\"lib:test\" #{current_file} -n #{method_name}")
      end
    end
  end
  ...
  def run_all
    if spec_file?
      ...
      write_output_to_buffer("ruby -I\"lib:test\" '#{current_file}'")
    end
  end
